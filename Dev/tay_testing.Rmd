...
replace_field <- function(df){
  # input_col <- enquos(input_col)
  # output_col <- enquos(output_col)
  
  df <- df %>%
    select(title_stocks, score)
  df <- df[!(is.na(df$title_stocks) | df$title_stocks==""), ]
  df <- separate_rows(df, title_stocks, sep=" ")
  
  # stocks <- df$input_col[!is.na(df$input_col)]
  # stocks <- paste(stocks, sep = " ")
  # stocks <- as.list(unlist(strsplit(stocks, '[[:space:]]')))
  # stocks <- unique(unlist(stocks)) #unique list of stocks
  

  stocks <- str_c("\\b", merged_common$Symbol, "\\b", collapse="|")
  
  df$sector <- str_replace_all(df$title_stocks, stocks, ticker_to_field)
  df <- df %>%
    select(-title_stocks)
...
test <- replace_field(head(wsb_dd_submissions, 1000))#, title_stocks, score)

```

```{r}
#test <- replace_field(head(wsb_dd_submissions, 1000))

wsb_dd_submissions %>%
  sample_n(1000) %>%
  replace_field(title_stocks, score) %>%
ggplot(aes(x=reorder(sector, score, FUN = mean), y=score)) +
  coord_flip() +
